...
const SlicePieWrapperForIneqaulParts = props => {
  ...
  const wrapper = c_values.map((item, index) => (
    <React.Fragment key={index}>
      <Slice
        clipnative
        slicedegree={120 * (index + 1) - props.pValue[p_values[index]]}
        ...
        <Pie
          ...
          widthoffset={props.widthoffset}
          piebg={props.paintShades[c_values[index]]}
          degree={300 + props.pValue[p_values[index]]}/>
      </Slice>
    </React.Fragment>
  ));
  return wrapper;
...
const Circle = props => {
  console.log('props in oneunit', props);
